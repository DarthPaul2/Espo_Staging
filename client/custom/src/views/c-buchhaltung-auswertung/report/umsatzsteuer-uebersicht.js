// Отчёт Umsatzsteuer-Übersicht.
// Что это: отдельный модуль рендера для auswertungTyp = umsatzsteuer_uebersicht.

define('custom:views/c-buchhaltung-auswertung/report/umsatzsteuer-uebersicht', [], function () {
    return {
                ...
                <div class="panel panel-default kb-auswertung-summary">
                    ...
                    <div class="panel-body">
                        <div class="row">
                            <div class="col-sm-3">
                                <div class="well">
                                    <div><strong>Anzahl Steuer-Buchungen</strong></div>
                                    <div class="kb-kpi-anzahl-buchungen" style="font-size: 22px;">0</div>
                                </div>
                            </div>
                            <div class="col-sm-3">
                                <div class="well">
                                    <div><strong>Summe Umsatzsteuer</strong></div>
                                    <div class="kb-kpi-ust-summe" style="font-size: 22px;">0,00 €</div>
                                </div>
                            </div>
                            <div class="col-sm-3">
                                <div class="well">
                            ...
                            <div class="col-sm-3">
                                <div class="well">
                                    <div><strong>Steuerkonten</strong></div>
                                    <div class="kb-kpi-steuerkonten" style="font-size: 22px;">0</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="panel panel-default kb-auswertung-tabs">
                    ...
                    <div class="panel-body">
                        ...
                        <div class="kb-tab-panel" data-tab-panel="gf">
                            ...
                            <p>Kompakter Überblick über Steuer-Buchungen der Ausgangsrechnungen.</p>
                        ...
                        <div class="kb-tab-panel hidden" data-tab-panel="buha">
                            ...
                            <p>Erweiterte Sicht auf Steuer-Buchungen mit Konto- und Journal-Bezug.</p>
        ...
        load(view) {
            ...
            view.getCollectionFactory().create('CBuchung', (collection) => {
                ...
                collection.fetch().then(() => {
                    ...
                    // Что это: оставляем только реальные Steuer-Buchungen
                    list = list.filter(item => {
                        const konto = String(item.kontoNummer || '').trim();
                        return konto === '3806' || konto === '3801';
                    });
                    ...
                    console.error('[UmsatzsteuerUebersicht] load failed', err);
                    view.notify('Fehler beim Laden der Umsatzsteuer-Übersicht', 'error');
                });
            });
        },
        ...
        render(view, list) {
            ...
            if (!list.length) {
                $tbodyGf.html('<tr><td colspan="6" class="text-muted">Keine Steuer-Buchungen gefunden.</td></tr>');
                $tbodyBuha.html('<tr><td colspan="8" class="text-muted">Keine Steuer-Buchungen gefunden.</td></tr>');
        ...
        updateKennzahlen(view, anzahlBuchungen, ustSumme, anzahlRechnungen, anzahlKonten) {
            view.$el.find('.kb-kpi-anzahl-buchungen').text(anzahlBuchungen);
            view.$el.find('.kb-kpi-ust-summe').text(view.formatCurrency_(ustSumme));
            view.$el.find('.kb-kpi-anzahl-rechnungen').text(anzahlRechnungen);
            view.$el.find('.kb-kpi-steuerkonten').text(anzahlKonten);
        }
    };
});