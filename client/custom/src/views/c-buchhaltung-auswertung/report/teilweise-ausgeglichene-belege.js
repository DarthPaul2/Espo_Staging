// Отчёт Teilweise ausgeglichene Belege.
// Что это: Phase-3-Bericht по документам, которые уже частично оплачены,
// но ещё не закрыты полностью.

define('custom:views/c-buchhaltung-auswertung/report/teilweise-ausgeglichene-belege', [], function () {
    return {
                        ...
                        <div class="row">
                            <div class="col-sm-3">
                                <div class="well">
                                    ...
                                    <div class="tb-kpi-anzahl" style="font-size: 22px;">0</div>
                                </div>
                            </div>
                            <div class="col-sm-3">
                                <div class="well">
                                    <div><strong>Summe ursprünglich</strong></div>
                                    <div class="tb-kpi-brutto" style="font-size: 22px;">0,00 €</div>
                                </div>
                            </div>
                            <div class="col-sm-3">
                                <div class="well">
                                    <div><strong>Bereits ausgeglichen</strong></div>
                                    <div class="tb-kpi-ausgeglichen" style="font-size: 22px;">0,00 €</div>
                                </div>
                            </div>
                            <div class="col-sm-3">
                                <div class="well">
                                    <div><strong>Restbetrag offen</strong></div>
                                    <div class="tb-kpi-rest" style="font-size: 22px;">0,00 €</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="panel panel-default kb-auswertung-tabs">
                    ...
                    <div class="panel-body">
                        <div class="alert alert-info kb-auswertung-info">
                            Zeitraum: <strong><span class="tb-info-zeitraum">–</span></strong>
                            &nbsp;|&nbsp;
                            Gefundene Belege: <strong><span class="tb-info-anzahl">0</span></strong>
                        </div>

                        <div class="kb-tab-panel" data-tab-panel="gf">
                            <p><strong>Geschäftsführung</strong></p>
                            <p>Kompakter Überblick über Belege, die bereits teilweise bezahlt wurden, aber noch offen sind.</p>
                                ...
                                <table class="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Fällig am</th>
                                            <th>Ursprünglich</th>
                                            <th>Bereits ausgeglichen</th>
                                            <th>Rest offen</th>
                                        </tr>
                                    </thead>
                                    <tbody class="tb-tbody-gf">
                                        <tr>
                        ...
                        <div class="kb-tab-panel hidden" data-tab-panel="buha">
                            ...
                            <p>Erweiterte Sicht auf teilweise ausgeglichene Ausgangs- und Eingangsbelege.</p>
        ...
        load(view) {
            ...
            this.loadRechnungen(view, zeitraumVon, zeitraumBis).then((rechnungen) => {
                this.loadEingangsrechnungen(view, zeitraumVon, zeitraumBis).then((eingangsrechnungen) => {
                    ...
                    console.error('[TeilweiseAusgeglicheneBelege] load Eingangsrechnungen failed', err);
                    view.notify('Fehler beim Laden der Eingangsrechnungen', 'error');
                });
            }).catch((err) => {
                console.error('[TeilweiseAusgeglicheneBelege] load Rechnungen failed', err);
        ...
        render(view, list) {
            const $tbodyGf = view.$el.find('.tb-tbody-gf');
            const $tbodyBuha = view.$el.find('.tb-tbody-buha');
            ...
            if (!list.length) {
                $tbodyGf.html('<tr><td colspan="7" class="text-muted">Keine teilweise ausgeglichenen Belege gefunden.</td></tr>');
                $tbodyBuha.html('<tr><td colspan="10" class="text-muted">Keine teilweise ausgeglichenen Belege gefunden.</td></tr>');
        ...
        updateKennzahlen(view, anzahl, brutto, ausgeglichen, rest) {
            view.$el.find('.tb-kpi-anzahl').text(anzahl);
            view.$el.find('.tb-kpi-brutto').text(view.formatCurrency_(brutto));
            view.$el.find('.tb-kpi-ausgeglichen').text(view.formatCurrency_(ausgeglichen));
            view.$el.find('.tb-kpi-rest').text(view.formatCurrency_(rest));
        },
        ...
        updateInfo(view, anzahl) {
            ...
            view.$el.find('.tb-info-zeitraum').text(zeitraumText);
            view.$el.find('.tb-info-anzahl').text(anzahl);
        },
