...
class PlzAutoFill
{
    ...
    {
        $plz = trim((string) $entity->get('plz'));

        if ($plz === '') {
            return;
        }

        $ort = (string) ($entity->get('ort') ?? '');
        $bundesland = (string) ($entity->get('bundesland') ?? '');
        $land = (string) ($entity->get('land') ?? '');
        ...
        if ($ort === '') {
            $entity->set('ort', $eintrag['ort']);
        }

        if ($bundesland === '') {
            $entity->set('bundesland', $eintrag['bundesland']);
        }

        if ($land === '') {
            $entity->set('land', 'Deutschland');
        }
    }
