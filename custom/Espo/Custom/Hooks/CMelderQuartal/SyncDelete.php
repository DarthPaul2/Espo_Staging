...
namespace Espo\Custom\Hooks\CMelderQuartal;
...
class SyncDelete
{
    ...
    // fallback (чтобы не трогать конфиги)
    ...
    public function afterRemove(Entity $entity, array $options = []): void
    {
        ...
        $this->log->info('[CMelderQuartal][afterRemove] start id=' . (string)$entity->getId() . ' sourceId=' . (string)$sourceId);

        if (!$sourceId) {
            $this->log->warning('[CMelderQuartal][afterRemove] sourceId empty -> skip');
            return;
        }
        ...
        $payload = json_encode([
            'entity'   => 'CMelderQuartal',
            'sourceId' => (string) $sourceId,
        ], JSON_UNESCAPED_UNICODE);

        $this->send($url, $secret, $payload, 'CMelderQuartal');
    }
